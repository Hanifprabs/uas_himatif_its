...
if ($_SESSION['type'] == "anggota") {
    ...
    if ($op == 'edit') {
        $id = $_GET['id'] ?? '';
        if ($id) {
            $sql1 = "SELECT * FROM kas_keluar WHERE id = ?";
    ...
    if (isset($_POST["simpan"])) {
        ...
        if (!$nama || !$tanggal || !$pengeluaran || !$jumlah || !$pengecek) {
            ...
            if ($op == 'edit' && isset($_GET['id'])) {
                $id = intval($_GET['id']);
                $sql1 = "UPDATE kas_keluar SET nama = ?, tanggal = ?, pengeluaran = ?, jumlah = ?, pengecek = ? WHERE id = ?";
                $stmt1 = $con->prepare($sql1);
                $stmt1->bind_param('sssssi', $nama, $tanggal, $pengeluaran, $jumlah, $pengecek, $id);
                $q1 = $stmt1->execute();
                ...
                $sql1 = "INSERT INTO kas_keluar (nama, tanggal, pengeluaran, jumlah, pengecek) VALUES (?, ?, ?, ?, ?)";
                $stmt1 = $con->prepare($sql1);
                $stmt1->bind_param('sssss', $nama, $tanggal, $pengeluaran, $jumlah, $pengecek);
                $q1 = $stmt1->execute();
    ...
    if ($op == 'delete' && isset($_GET['id'])) {
        $id = $_GET['id'];
        if ($id) {
            $sql_delete = "DELETE FROM kas_keluar WHERE id = ?";
                                    ...
                                    <form action="" class="forms-sample" method="POST">
                                        <div class="form-group">
                                            ...
                                            <input type="text" class="form-control" id="exampleInputName1" name="nama"
                                                placeholder="Name" value="<?= htmlspecialchars($nama ?? ''); ?>">
                                        </div>
                                        <div class="form-group">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <input type="text" class="form-control" id="exampleInputPassword4" name="pengeluaran"
                                                placeholder="Pengeluaran" value="<?= htmlspecialchars($pengeluaran ?? ''); ?>">
                                        </div>
                                        <div class="form-group">
                                            <label for="exampleInputCity1">Jumlah</label>
                                            <input type="text" class="form-control" id="exampleInputCity1" name="jumlah"
                                                placeholder="Jumlah" value="<?= htmlspecialchars($jumlah ?? ''); ?>">
                                        </div>
                                        <div class="form-group">
                                            <label for="exampleInputCity1">Pengecek</label>
                                            <input type="text" class="form-control" id="exampleInputCity1" name="pengecek"
                                                placeholder="Pengecek" value="<?= htmlspecialchars($pengecek ?? ''); ?>">
                                        </div>
                            ...
                            <div class="card">
                                <div class="card-body">
                                    ...
                                    <form action="" method="GET">
                                        ...
                                        <div class="input-group mb-2 mr-sm-2">
                                            <input type="text" class="form-control" placeholder="Cari nama atau pengecek" name="cari"
