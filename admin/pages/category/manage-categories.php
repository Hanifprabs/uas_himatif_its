...
if ($op == 'edit') {
    $id = $_GET['id'] ?? '';
    if ($id) {
        $sql1 = "SELECT * FROM tblcategory WHERE id = ?";
...
if (isset($_POST["simpan"])) {
    ...
    if (!$CategoryName || !$Description) {
        ...
        if ($op == 'edit' && isset($_GET['id'])) {
            $id = intval($_GET['id']);
            $sql1 = "UPDATE tblcategory SET CategoryName = ?, Description = ?, UpdationDate = NOW() WHERE id = ?";
            $stmt1 = $con->prepare($sql1);
            $stmt1->bind_param('ssi', $CategoryName, $Description, $id);
            $q1 = $stmt1->execute();
            ...
            $sql1 = "INSERT INTO tblcategory (CategoryName, Description, PostingDate, UpdationDate, Is_Active) VALUES (?, ?, NOW(), NOW(), 1)";
            $stmt1 = $con->prepare($sql1);
            $stmt1->bind_param('ss', $CategoryName, $Description);
            $q1 = $stmt1->execute();
...
if ($op == 'delete' && isset($_GET['id'])) {
    $id = $_GET['id'];
    if ($id) {
        $sql_delete = "DELETE FROM tblcategory WHERE id = ?";
...
if (isset($_GET['cari']) && $_GET['cari'] != '') {
    ...
    $where[] = "(CategoryName LIKE '%$cari%' OR Description LIKE '%$cari%')";
}
$whereSQL = '';
if (count($where) > 0) {
    $whereSQL = 'WHERE ' . implode(' AND ', $where);
}
$sql2 = "SELECT * FROM tblcategory $whereSQL ORDER BY id DESC";
$q2 = $con->query($sql2);
                ...
                <div class="card-body">
                  ...
                  <form action="" class="forms-sample" method="POST">
                    ...
                    <div class="form-group">
                      ...
                      <textarea class="form-control" id="Description" name="Description" rows="4" placeholder="Category Description"><?= htmlspecialchars($Description ?? ''); ?></textarea>
                    </div>
                ...
                <div class="card-body">
                  ...
                  <form action="" method="GET">
                    <div class="input-group mb-2 mr-sm-2">
                      <input type="text" class="form-control" placeholder="Cari kategori atau deskripsi" name="cari"
                    ...
                    <table class="table table-striped">
                      ...
                      <tbody>
                        ...
                        <tr>
                          ...
                          <td>
                            <a href="manage-kategori.php?op=edit&id=<?php echo $id; ?>"><button type="button"
                                class="badge badge-warning">Edit</button></a>
                            <a href="manage-kategori.php?op=delete&id=<?php echo $id; ?>"
                      ...
                      <table class="table table-bordered table-contextual">
                        <thead>
                          <tr class="table-warning">
                            <td>Total Kategori Sesuai Filter: </td>
                            <td><?= $q2->num_rows; ?></td>
                          </tr>
                          <tr class="table-danger">
                            <td>Total Kategori Keseluruhan: </td>
                            <td><?php
                              $totalSql = "SELECT COUNT(*) AS total FROM tblcategory";
